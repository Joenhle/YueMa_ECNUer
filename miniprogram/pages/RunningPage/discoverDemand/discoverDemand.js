...
Page({
  ...
  searchPost: function (e) {
    ...
    for (let i = vardemandposts.length - 1; i >= 0; i--) {
      ...
      if (speed[speed.length - 1] != 5) { //删除速度不符合查找要求的
        ...
        for (let k = 0; k < speed.length; k++) {
          if (speed[k] == vardemandposts[i].speed_require) {
            havespeed = 1;
            break;
          }
    ...
    console.log('根据需求搜索帖子', vardemandposts)
    wx.showToast({
      title: '搜索需求成功',
  ...
  getListIndexSkip: function (skip) {
    return new Promise((resolve, reject) => {
      ...
      let selectPromise;
      if (skip > 0) {
        selectPromise = db.collection('DemandPosts').skip(skip).get()
      } else {
        //skip值为0时，会报错
        selectPromise = db.collection('DemandPosts').get()
      }
