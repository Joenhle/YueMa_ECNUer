// miniprogram/pages/PlayingballPage/ballDemandPost/ballDemandPost.js
...
Page({
  ...
  onLoad: function (options) {
    ...
    db.collection('BallDemandPosts').doc(post_id).get({
      success: function (res) {
    ...
    //4. 再实时的进行数据库数据监控
    const watcher = db.collection('BallDemandPosts').doc(that.data.post_id)
      .watch({
  ...
  Jion_running: function (e) {
    ...
    var item = { //存进订阅消息
      ...
      thing1: {
        value: that.data.ball[that.data.demandPost.ball]
      },
      thing4: {
      ...
      db.collection('BallDemandPosts').doc(post_id).get({
        success: function (dbres) {
          ...
          if (num_of_people < max_num) {
            ...
            var run_time = temppost.run_time //打球时间
            var finish_time = temppost.finish_time //打球结束时间
            ...
            console.log('需求帖子用户数组人数', now_num)
            //调用云函数 db_jionPlayingBall
            wx.cloud.callFunction({
              name: 'db_jionPlayingBall',
              data: {
              ...
              success: res => {
                ...
                console.log('加入约球云函数返回值', res)
                //成功加入，将信息保存到mine数据库里
                const db = wx.cloud.database();
                db.collection('mine').doc(user_id).get({
                  success: function (mires) {
                    var myposts = mires.data.balldemandposts; //参加的运动帖子数组
                    var yuetime = mires.data.yueqiu; //打球次数
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          if (that.data.openid == that.data.demandPost._openid) {
            const db = wx.cloud.database();
            db.collection('BallDemandPosts').doc(post_id).remove({
              success: mineres => {
            ...
            const db = wx.cloud.database();
            db.collection('BallDemandPosts').doc(post_id).update({
              data: {
                members: members,
                num_of_people: num_of_people,
              },
              success: dpres => {
                console.log('balldemandposts删除一人成功', dpres)
              },
              fail: err => {
                wx.showToast({
                  icon: 'none',
                ...
                console.error('[数据库]balldemandpos[更新记录] 失败：', err)
              }
            })
          }
          if (!is_history) {
          ...
          const db = wx.cloud.database();
          db.collection('mine').doc(_id).get({ //先获取mine里的运动帖子数组
            success: function (mires) {
              var myposts = mires.data.balldemandposts; //参加的运动帖子数组
              var yuetime = mires.data.yueqiu; //运动次数
